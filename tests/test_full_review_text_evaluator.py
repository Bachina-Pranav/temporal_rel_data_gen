...
def test_evaluator_reports_review_text_and_cross_field_metrics(tmp_path):
    real = pd.DataFrame(
        {
    ...
    config = ConditionalTABDLMConfig(raw={"paths": {"output_dir": str(tmp_path)}, "evaluation": {"review_text_privacy_sample_size": 2}}, schema=schema)
    ...
    assert "summary_review_text_rougeL_mean" in metrics["text_consistency"]
