...
def test_sampling_reduced_steps_handles_final_partial_batch():
    frame = pd.DataFrame(
        {
    ...
    assert len(attrs["rating"]) == 3
    assert len(attrs["summary"]) == 3


def test_masked_denoising_schedule_preserves_full_step_reveal_probabilities():
    schedule = masked_denoising_schedule(total_timesteps=5, sampling_steps=5)
    assert schedule == [5, 4, 3, 2, 1]
    probs = [reveal_probability_for_schedule(cur, nxt) for cur, nxt in zip(schedule, schedule[1:] + [0])]
    assert probs == pytest.approx([1 / 5, 1 / 4, 1 / 3, 1 / 2, 1.0])
    reduced = masked_denoising_schedule(total_timesteps=50, sampling_steps=5)
    assert reduced[0] == 50
    assert reduced[-1] == 1
    assert len(reduced) == 5
    assert masked_denoising_schedule(total_timesteps=50, sampling_steps=1) == [50]


def test_sample_logits_top_k_only_returns_top_k_ids():
    import torch

    logits = torch.tensor([[0.0, 1.0, 2.0, 3.0]])
    draws = [int(sample_logits(logits, top_p=1.0, top_k=2).item()) for _ in range(20)]
