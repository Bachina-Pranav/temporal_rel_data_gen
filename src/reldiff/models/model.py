...
from reldiff.models.transformer import Reconstructor, Tokenizer, Transformer
import torch
...
class MLPDiffusion(nn.Module):
    def __init__(self, d_in, dim_t=512, d_out=None, use_mlp=True):
        super().__init__()
        self.dim_t = dim_t
        if d_out is None:
            d_out = d_in
        ...
        self.mlp = (
            nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(dim_t, d_out),
            )
            if use_mlp
            else nn.Linear(dim_t, d_out)
        )
...
class UniModMLP(nn.Module):
    ...
    def forward(self, x_num, x_cat, timesteps, condition=None):
        e = self.tokenizer(x_num, x_cat)
        decoder_input = e[:, 1:, :]  # ignore the first CLS token.
        y = self.encoder(decoder_input)
        pred_y = self.mlp(y.reshape(y.shape[0], -1), timesteps, condition)
